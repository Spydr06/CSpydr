...
static void C99_generate_stmt(CodegenData_T* c, IRStmt_T* stmt)
{
    switch(stmt->kind)
    {
        ...
        case IR_STMT_STORE_DEREF:
            ...
            break;
        case IR_STMT_LABEL:
            P_ln("_L%u:", stmt->label.id);
            break;
        case IR_STMT_GOTO:
            P_ln("  goto _L%u;", stmt->_goto.label_id);
            break;
        case IR_STMT_GOTO_IF:
            ...
            C99_generate_literal(c, &stmt->goto_if.condition);
            P_ln(") goto _L%u;", stmt->goto_if.label_id);
            break;
        default:
    }
}
...
{\n\
    return " MANGLE_STATIC_PREFIX "main((int32_t) argc, (int8_t**) argv);\n\
}"));
